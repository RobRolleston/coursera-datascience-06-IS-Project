...
Compare different doses of supp
```{r}
...
```
### Conclusions
The only case where changing the supp or the dose does not affect the len is when the dose for each supp==2 (p-vale=0.96).  In all other cases the p-value is less than 0.05, and thus the null hypotheseis is rejected.
